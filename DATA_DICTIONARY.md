...
physical unit. All force/moment resultant arrays use base-SI numeric values.
They are expressed as `Pa m` for axial and shear resultants, and `Pa m^2` for bending moments.
Source/provenance classes are **synthetic reference**,
...
| `profiles_NM.npz:N`, `N_hooke`, `N_grid` | each `(100, 200)` / `float64` | Pa m | Axial-resultant reference and two constitutive/grid comparison routes; synthetic reference. |
| `profiles_NM.npz:M`, `M_hooke`, `M_grid` | each `(100, 200)` / `float64` | Pa m^2 | Bending-moment reference and two comparison routes; synthetic reference. |
| `profiles_NM.npz:Q` | `(100, 200)` / `float64` | Pa m | Shear resultant; synthetic reference. |
...
| `primary_test_predictions:axial_force_truth`, `axial_force_prediction`, `axial_force_prediction_strain_route` | `(20, 200)` / `float64`, `float32`, `float64` | Pa m | Reference, direct model, and strain-route axial resultants; synthetic reference/model prediction/processed analysis. |
| `primary_test_predictions:bending_moment_truth`, `bending_moment_prediction_direct`, `bending_moment_prediction_strain_route` | `(20, 200)` / `float64`, `float32`, `float64` | Pa m^2 | Reference, direct model, and strain-route bending resultants; synthetic reference/model prediction/processed analysis. |
...
| Same ablation archives: `predicted_axial_force` | `(20, 200)` / `float32` | Pa m | Ablation axial-resultant prediction. |
| Same ablation archives: `predicted_bending_moment_direct` | `(20, 200)` / `float32` | Pa m^2 | Ablation bending-moment prediction. |
