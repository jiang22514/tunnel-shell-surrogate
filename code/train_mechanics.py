#!/usr/bin/env python3
"""Train the six mechanics heads in the paper's primary configuration."""
...
FIELDS = ("eps_m", "slope_ss", "enn_c0", "enn_slope", "N", "M")
...
def main() -> None:
    ...
    print(
        f"six-head mechanics training finished in "
        ...
        for local_index, case_index in enumerate(test_indices):
            ...
            numerator = np.sum(
                (
                    ...
                    - profiles[field][case_index][case_mask]
                )
                ** 2
            )
            denominator = (
                np.sum(profiles[field][case_index][case_mask] ** 2) + 1.0e-30
            )
            errors.append(100.0 * np.sqrt(numerator / denominator))
        print(
            f"{field:9s} profile relative L2: "
            f"median {np.median(errors):.2f}% "
            f"mean {np.mean(errors):.2f}% max {np.max(errors):.2f}%",
            flush=True,
        )

    args.output_dir.mkdir(parents=True, exist_ok=True)
    stem = f"ncs{args.seed}_nobc_nopca"
    torch.save(
        {field: network.state_dict() for field, network in networks.items()},
        args.output_dir / f"_stage3_shell_model_{stem}.pt",
    )
    np.savez(args.output_dir / f"_stage3_shell_pred_{stem}.npz", **output)
